        ...
        {posts.map((post, i) => (
          <div key={post.id} className="flex gap-3 group">
            <span className="text-lg font-black text-primary/30 leading-none mt-0.5 shrink-0">
              {String(i + 1).padStart(2, "০".charCodeAt(0) > 48 ? "0" : "0")}
              {/* Bangla number */}
              {"০১২৩৪৫৬৭৮৯"[i + 1] || (i + 1)}
            </span>
            <div className="flex gap-2 flex-1">
              ...
              <h4 className="text-[11px] font-semibold leading-relaxed text-foreground group-hover:text-primary transition-colors cursor-pointer line-clamp-2">
                {post.title}
              </h4>
            </div>
          </div>
