...
export default function LabelPostSection({ label, count, layout = "list" }: Props) {
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="md:col-span-2 post-card bg-card rounded overflow-hidden shadow-sm">
            ...
            <div className="p-4">
              <h3 className="text-lg font-bold leading-relaxed text-foreground hover:text-primary transition-colors cursor-pointer">
                {posts[0].title}
              </h3>
  ...
  return (
    <section>
      <SectionLabel label={label} />
      <div className="space-y-3">
        <div className="post-card bg-card rounded overflow-hidden shadow-sm">
          <div className="overflow-hidden aspect-video">
            <img src={posts[0].image} alt={posts[0].title} className="w-full h-full object-cover post-image" />
          </div>
          <div className="p-4">
        ...
        {posts.slice(1).map((post) => (
          <div key={post.id} className="post-card flex gap-3 bg-card rounded p-2 shadow-sm">
            ...
            <div>
              <h4 className="text-sm font-semibold leading-relaxed text-foreground hover:text-primary transition-colors cursor-pointer line-clamp-2">
                {post.title}
              </h4>
              <span className="text-[10px] text-muted-foreground">{post.date}</span>
            </div>
